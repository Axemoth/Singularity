...
export async function POST(request: NextRequest) {
  ...
  if (!tenantId || tenantId === "dev") {
    if (
      ...
      try {
        const dataStr = Buffer.from(body.message.data, "base64").toString(
          "utf8",
        );
        ...
        if (emailAddress) {
          ...
          if (parseResult.success) {
            ...
            if (accountRow) {
              ...
              console.info(
                `[Webhook] Resolved tenantId "${tenantId}" from Gmail Pub/Sub emailAddress "${validatedEmail}"`,
              );
            } else {
              console.warn(
                `[Webhook] No tenant account found matching emailAddress "${validatedEmail}"`,
              );
            }
          } else {
            console.error(
              `[Webhook] Invalid email address format received in Pub/Sub payload:`,
              emailAddress,
            );
          }
        }
        ...
        console.error(
          "[Webhook] Failed to resolve tenantId from Pub/Sub payload:",
          err,
        );
      }
    }
