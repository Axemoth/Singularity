...
export function ComposePanel() {
  ...
  const toggleListening = useCallback(() => {
    ...
    if (isListening) {
      ...
      inputBeforeSpeechRef.current = aiInput;
      ...
      recognition.onresult = (event: any) => {
        ...
        setAiInput(baseText + spacing + transcript);
      };
  ...
  }, [isListening, speechLanguage, aiInput]);
      ...
      <aside className="border-border-default bg-bg-raised animate-slide-right flex h-full w-full max-w-[840px] border-l shadow-[var(--shadow-xl)] overflow-hidden">
        ...
        <section className="w-1/2 flex flex-col bg-bg-base/40">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
            {aiMessages.map((message) => {
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[90%] rounded-[var(--radius-md)] px-3 py-2 text-[12.5px] leading-relaxed whitespace-pre-wrap break-words ${
                      message.role === "user"
          ...
          <div className="border-t border-border-subtle p-3 bg-bg-raised">
            ...
            {/* Input Form */}
            <form onSubmit={handleChatSubmit} className="flex flex-col gap-2 rounded-xl border-none bg-bg-surface p-2 shadow-sm focus-within:shadow-md transition-all focus-within:bg-bg-base outline-none focus-within:outline-none focus-within:ring-0">
              <textarea
                ref={chatInputRef}
                value={aiInput}
                ...
                className="bg-transparent text-text-primary placeholder:text-text-tertiary w-full resize-none px-2 py-1 text-xs outline-none border-none focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0"
